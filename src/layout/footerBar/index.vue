...
<template>
  <footer class="dark:bg-[#25272c] fixed bottom-0 left-0 z-50 w-full bg-white h-[60px]">
    <div class="grid border-t py-1 dark:border-t-neutral-800 grid-cols-2 h-full  items-center" :style="{	gridTemplateColumns: `repeat(${menuLista.length}, minmax(0, 1fr))` }">
      <a v-for="(item) in menuLista" :key="item.id" :class="[isActive(item) ? 'text-[#7869f5] dark:text-[#7869f5]' : '']" class="cursor-pointer text-center leading-4 " @click="handleToPage(item)">
        <span class="inline-block text-xl"> <SvgIcon :icon="item.menuIcon" class="mb-1 inline-block text-lg" /></span>
        <p class="text-xs mb-1">{{ item.menuTipText }}</p>
      </a>
    </div>
  </footer>
</template>
