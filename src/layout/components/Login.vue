...
<template>
  <NModal :show="visible" :on-after-enter="openDialog" :on-after-leave="handleCloseDialog">
    <div class="w-[1080px] h-[600px] bg-transparent rounded-[13px] overflow-hidden dark:bg-slate-800" :class="isMobile ? 'px-[5px]' : ''">
      ...
      <div class="bg-transparent m-0 flex ">
        <div v-if="!isMobile" class="w-[500px] h-[600px] overflow-hidden">
          <img :src="loginDialogImg" class=" w-full h-full">
        </div>
