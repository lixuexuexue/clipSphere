...
const signInModelCount = computed(() => Number(authStore.globalConfig?.signInModelCount) || 0)
  ...
  <NModal :show="visible" :style="{ maxWidth: '650px', minWidth: isSmallLg ? '100%' : '650px' }" :on-after-enter="openDrawerAfter" :on-after-leave="handleCloseDialog">
    <NSpace vertical>
      <NCard closable @close="handleClose">
        ...
        <NAlert class="mb-5 p-0 dark:bg-[#1c2026] bg-[#e0e2eb]" :show-icon="false" type="primary">
          每日签到赠送：
          <span v-if="signInModelCount > 0"><b class=" text-[red]">{{ signInModelCount }}</b>积分对话模型对话额度</span>
        ...
        <NSpin :show="loading">
          <NCalendar v-model:value="value" style="height:420px" #="{ month, date }" :is-date-disabled="isDateDisabled">
            <div v-if="signed(month, date)" class="flex items-center w-full mt-2">
              <SvgIcon icon="heroicons:gift" class="text-xl text-[#8E6EF7]" />
