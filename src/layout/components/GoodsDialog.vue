...
import { NButton, NGrid, NGridItem, NIcon, NModal, NScrollbar, NSkeleton, NSpace, useDialog, useMessage } from 'naive-ui'
...
<template>
  <NModal :show="visible" :style="{ maxWidth: `${packageList.length > 3 ? 1200 : packageList.length * 300}px`, minWidth: isSmallMd ? '100%' : '1150px' }" :on-after-enter="openDialog" :on-after-leave="handleCloseDialog">
    <div class="p-4 bg-white rounded dark:bg-slate-800 max-h-4/5">
      <div class=" flex cursor-pointer justify-between ">
        <span class="text-xl mb-3">选购商品</span>
        <NIcon size="20" color="#0e7a0d" @click="useGlobalStore.updateGoodsDialog(false)">
          <CloseOutline />
        </NIcon>
      </div>
      <NScrollbar x-scrollable>
        <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4  gap-x-6 gap-y-10 px-4 ">
          <div v-for="(item, index) in packageList" :key="index" class="h-full flex-shrink-0 rounded-[10px] overflow-hidden dark:bg-[#0F1416] flex flex-col">
            <img :src="item.coverImg" class="w-full h-[160px] object-fill">
            <div class="relative p-3 text-[16px]">
              <b>{{ item.name }}</b>
            ...
            <div class="flex flex-col justify-between px-3">
              <p class="h-[60px] text-[12px] line-clamp-3 break-all">
                {{ item.des }}
              </p>
              <div class="flex flex-1 justify-between items-end min-h-28">
                <span class="text-sm font-bold mr-1 w-[120px]">对话模型额度</span>
                <span class="font-bold">{{ item.modelCount }}</span>
              </div>
