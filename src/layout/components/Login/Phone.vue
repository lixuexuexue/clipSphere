  ...
  formRef.value?.validate(async (errors) => {
    if (!errors) {
      try {
        ...
        const params: any = !isLogin.value ? { phone: registerForm.value.phone, password: registerForm.value.password, phoneCode: registerForm.value.phoneCode } : { phone: loginForm.value.phone, password: loginForm.value.password }
...
<template>
  <div class="px-[65px]" :class="isLogin ? 'pt-[40px]' : 'pt-5'">
    ...
    >
      <Motion :delay="190">
        <NFormItem path="phone">
          <NInput v-model:value="registerForm.phone" autocomplete="off" placeholder="请填写您的手机号" />
        </NFormItem>
      </Motion>
      <Motion :delay="120">
      ...
      <Motion :delay="330">
        <NFormItem v-if="isSendCaptcha" path="phoneCode">
          <NInput v-model:value="registerForm.phoneCode" autocomplete="off" class="flex-1" placeholder="请填写手机验证码" />
        </NFormItem>
      </Motion>
    ...
    >
      <Motion :delay="50">
        <NFormItem path="phone">
          <NInput v-model:value="loginForm.phone" autocomplete="off" placeholder="请输入手机号" />
        </NFormItem>
      </Motion>
      <Motion :delay="120">
        <NFormItem path="password">
  ...
  <div class="flex items-center justify-center space-x-5 " :class="phoneRegisterStatus ? 'mt-[16px]' : 'mt-[36px]'">
    ...
    <NButton v-if="emailLoginStatus" ghost class="!px-10" @click="emit('changeLoginType', 'email')">
      <SvgIcon class="text-xl mr-2 text-[#7869f5]" icon="clarity:email-line" />
      邮箱号登录
    </NButton>
  </div>
